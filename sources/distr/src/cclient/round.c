...
void lp_optimize_hr_round_decreasing_low_comm(dataset_histogram *hr, int servers,
	...
	bool improve_exchange, double *residual) {
...
void compute_residual(multiway_histogram_estimate *agg_server, double *residual, int servers) {
	double resmin = agg_server[1].to_pnts;
	for(int s = 2; s <= servers; s++) {
		if (resmin > agg_server[s].to_pnts)
			resmin = agg_server[s].to_pnts;
	}
	for(int s = 1; s <= servers; s++) {
		residual[s-1] = agg_server[s].to_pnts - resmin;
	//	residual[s-1] = 0;
	}
}
