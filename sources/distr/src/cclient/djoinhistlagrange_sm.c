...
double get_sm_objective(dataset_histogram *hr, optimization_data_s *opt_data, int opt_atu,
	double f, int servers, double multiplier, double *x0_ub, double *x0_lb, double *fmkspan, 
	double *fcomm, double *residual) {
	double x[servers+1];
	memset(x, 0, sizeof x);
	double netcost = 0.0;
	double cpucost = 0.0;
...
double find_knapsack_bestf_capacity(optimization_data_s *opt_data, int pairs, int servers, double multiplier, double *residual) {
	int points[pairs+servers];
	for(int i = 0; i < pairs; i++)
		points[i] = ceil(opt_data[i].pnts * multiplier);
	for(int s = 0; s < servers; s++)
		points[pairs+s] = residual[s];
	qsort(points, pairs, sizeof(int), int_decreasing_compare);	

	double serverload[servers];
	memset(serverload, 0, sizeof serverload);
	for(int i = 0; i < pairs + servers; i++) {
		int min = 0;
...
void lagrange_sm_optimize_hr(dataset_histogram *hr, int servers,
	...
	// residual load on machines
	double residual[servers];
	compute_residual(agg_server, residual, servers);
	for(int s=0; s<servers; s++) {
		printf("residual[%d]: %f\n", s, residual[s]);
	}
	...
	double multiplier = 1;
	knapsack_bestf = find_knapsack_bestf_capacity(opt_data, pairs, servers, 1, residual);
	...
	printf("Knapsack bestf %f: multiplier %e, f=%f\n", knapsack_bestf, multiplier, f);

	double heur_x0;
	double x0_lb;
	double Zheur = get_sm_objective(hr, opt_data, pairs, md.f, servers, multiplier, &heur_x0, &x0_lb, NULL, NULL, residual);
...
//	const int stop = 10000;
	...
	knapsack_bestf = find_knapsack_bestf_capacity(opt_data, pairs, servers, multiplier, residual);
	...
	while (k < stop) {
		...
		for(int s = 0; s < servers; s++) {
			double cap = MAX(0, knapsack_atu - residual[s]);
			if (cap == 0) { 
				// residual occupies the entire knapsack
				// no item will be selected
				for(int j=0; j < pairs; j++)
					x_ijk[s][j] = 0;
			}
			else {
				int z = minknap(pairs, profit[s], weight, x_ijk[s], cap);
				//if (z == 0)
		...
		double x0;
		Zdk = lagrange_sm_get_opt_value(&md, pairs, servers, x_ijk, opt_data, &x0, residual);
		...
		if (best_processed < processed || (best_processed == processed && Zdk > zd_processed)) {
			...
			lp_optimize_hr_round_decreasing_low_comm(hr, servers, opt_data, pairs, x_ijk, agg_server, md.f, false, false, residual);
			double NewZheur = get_sm_objective(hr, opt_data, pairs, md.f, servers, multiplier, &heur_x0, NULL, NULL, NULL, residual);
			if (NewZheur < Zheur) {
				choosed = 'x';
	...
	// improve exchange
	lp_optimize_hr_round_decreasing_low_comm(hr, servers, opt_data, pairs, best_x_ijk, agg_server, md.f, false, true, residual);
	//set_cell_place_from_partial_x_nowh(hr, servers, pairs, best_x_ijk, opt_data);

	Zheur = get_sm_objective(hr, opt_data, pairs, md.f, servers, multiplier, NULL, NULL, &final_mkspan, &final_comm, residual);
