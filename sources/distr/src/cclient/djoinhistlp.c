...
void lp_optimize_hr(dataset_histogram *hr, int servers,
	...
	Zheur = get_sm_objective(hr, opt_data, opt_atu, f, servers, 1, NULL, NULL, &final_mkspan, &final_comm, residual);
	printf("Before LP rounding\nZ\tMkspan\tComm\n%.2f\t%.2f\t%.2f\n", Zheur, final_mkspan, final_comm);

	lp_optimize_hr_round_decreasing_low_comm(hr, servers, opt_data, opt_atu, xaux, agg_server, f, true, false, residual);
