...
//	improve_transformed_solution_fm(servers, opt_data, pairs, load, where, x, f);
	...
	double Zheur = get_sm_objective(hr, opt_data, pairs, f, servers, 1, NULL, NULL, &final_mkspan, &final_comm, residual);
...
void bs_optimize_hr_old(dataset_histogram *hr, int servers,	optimization_data_s *opt_data, 
	...
	double final_mkspan, final_comm;
	double Zheur = get_sm_objective(hr, opt_data, opt_atu, servers, servers, 1, NULL, NULL, &final_mkspan, &final_comm, residual);
	printf("After BS\nZ\tMkspan\tComm\n%.2f\t%.2f\t%.2f\n", Zheur, final_mkspan, final_comm);
